...
void test_sha384() {
	...
	puts("\n\nTesting SHA384...\n");
	for (i = 0; i < 4; i++) {
		sha384_simple(tests[i], strlen(tests[i]), hash);
...
void test_sha384_stream() {
	...
	puts("\n\nTesting SHA384...\n");
	for (i = 0; i < 4; i++) {
		sha384_state_t hash_state;
		sha384_init(&hash_state);
		for (j = 0; j < (int)strlen(tests[i]); j++) {
			printf("%c.", tests[i][j]);
			sha384_process(&hash_state, &tests[i][j], 1);
		}
		printf("\n");
		sha384_done(&hash_state, hash);
		sha384_hash_to_string(hash, string);
		printf("%s\n%s\n--> %s\n\n", tests[i], string, strcmp(string, oks[i]) == 0 ? "OK" : "FAIL");
	}
	puts("\nTest done.\n");
...
void test_sha512() {
	...
	puts("\n\nTesting SHA512...\n");
	for (i = 0; i < 4; i++) {
		sha512_simple(tests[i], strlen(tests[i]), hash);
...
void test_sha512_stream() {
	...
	puts("\n\nTesting SHA512...\n");
	for (i = 0; i < 4; i++) {
		sha512_state_t hash_state;
		sha512_init(&hash_state);
		for (j = 0; j < (int)strlen(tests[i]); j++) {
			printf("%c.", tests[i][j]);
			sha512_process(&hash_state, &tests[i][j], 1);
		}
		printf("\n");
		sha512_done(&hash_state, hash);
		sha512_hash_to_string(hash, string);
		printf("%s\n%s\n--> %s\n\n", tests[i], string, strcmp(string, oks[i]) == 0 ? "OK" : "FAIL");
	}
	puts("\nTest done.\n");
