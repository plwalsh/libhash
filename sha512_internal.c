...
void sha512_compress(sha512_state_t * s, word_t res[BLOCK_SIZE]);
...
void sha512_compress(sha512_state_t * s, word_t res[BLOCK_SIZE]) {
    ...
    for (i = 0, j = 0; i < 16; i++, j += (unsigned int)WORD_SIZE) {
...
void sha512_compute(sha512_state_t * s, const uint8_t data[], const size_t size) {
    ...
    //compressing
    for (i = 0; i < size; i++) {
        s->buf[s->curlen] = data[i];
        s->curlen++;
        if (s->curlen == BLOCK_SIZE) {
            sha512_compress(s, s->buf);
            s->length += BLOCK_SIZE_BITS;
            s->curlen = 0;
        }
    }
}
...
void sha512_conclude(sha512_state_t * s, uint8_t hash[], const size_t hash_size) {
    ...
    //add padding
    if (s->curlen < BLOCK_SIZE - 8) {
        s->buf[s->curlen++] = 0x80;
        while (s->curlen < BLOCK_SIZE - 8) {
            s->buf[s->curlen++] = 0;
        }
    } else {
        s->buf[s->curlen++] = 0x80;
        while (s->curlen < BLOCK_SIZE) {
            s->buf[s->curlen++] = 0;
        }
        sha512_compress(s, s->buf);
        memset(s->buf, 0, BLOCK_SIZE - 8);
    }

    //append length
    unsigned int i;
    for (i = 0; i < 8; i++) {
        s->buf[BLOCK_SIZE - 1 - i] = s->length >> (i * 8);
    }
    sha512_compress(s, s->buf);
    ...
    for (i = 0; i < WORD_SIZE; i++) {
        for (j = 0; j < hash_size / WORD_SIZE; j++) {
            hash[i + WORD_SIZE * j] = (uint8_t)(s->state[j] >> (WORD_SIZE_BITS - 8 - i * 8));
        }
    }
}
